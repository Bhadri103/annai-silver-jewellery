  ...
  <p className="flex items-center gap-3">
    ...
    <span>Comfortable for everyday wear.</span>
        ...
        <section className="mt-8">
          ...
          <div className="mt-5 grid grid-cols-3 gap-3 sm:gap-5">{related.map(item => <Link key={item.name} to={`/product/${productSlug(item.name)}`} className="overflow-hidden rounded-2xl border border-amber-100 bg-white"><img src={item.image} alt={item.name} className="h-40 w-full object-contain transition hover:scale-105 sm:h-72" /><div className="p-3 sm:p-5"><h3 className="text-xs font-semibold sm:text-base">{item.name}</h3><p className="mt-2 text-xs font-semibold text-amber-700 sm:text-sm"><Price value={item.price} /></p></div></Link>)}</div></section>
