...
export default function SupplementCartPage() {
  ...
  const catalogue = useMemo(() => productShelves.flatMap((shelf) => shelf.products), []);
  ...
  }).filter((item): item is { key: string; product: Product; quantity: number } => Boolean(item)), [cart, catalogue, savedProducts]);
