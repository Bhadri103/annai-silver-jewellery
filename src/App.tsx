...
import AdminPanel from "./admin/HighgradeAdminPanel";
import Footer from "./components/Footer";
import Header from "./components/Header";
import HighgradeChatBot from "./components/HighgradeChatBot";
...
const AppContent = () => {
  ...
  const isAdminRoute = location.pathname.startsWith("/highdrade-admin-control") || location.pathname.startsWith("/highgrade-admin-control");
  const showBackButton = !isAdminRoute && location.pathname !== "/";
  return <>
    <ScrollToTop />
    <SiteMotion pageKey={location.pathname} />
    {!isAdminRoute && <ShortScrollbar />}
    <div className="min-h-screen bg-white text-amber-900">
      ...
      <main>
        ...
        <Routes>
          <Route path="/highdrade-admin-control" element={<AdminPanel />} /><Route path="/highgrade-admin-control" element={<AdminPanel />} />
          <Route path="/" element={<HomePage />} /><Route path="/about" element={<AboutPage />} /><Route path="/collection/:collectionId" element={<CollectionPage />} /><Route path="/product/:slug" element={<ProductDetailPage />} /><Route path="/supplements" element={<ProgramDetailPage />} />
          ...
          <Route path="/success-stories" element={<SuccessStoriesPage />} /><Route path="/gallery" element={<GalleryPage />} /><Route path="/timetable" element={<TimetablePage />} /><Route path="/booking" element={<BookingPage />} />
          <Route path="/contact" element={<ContactPage />} /><Route path="/contact-us" element={<ContactPage />} /><Route path="/faq" element={<FAQPage />} /><Route path="/calculators" element={<CalculatorsPage />} />
          <Route path="/highgradeacademy/verify/:token" element={<CertificateVerifyPage />} /><Route path="/member-portal" element={<MemberPortalPage />} /><Route path="*" element={<HomePage />} />
        </Routes>
        </div>
      </main>
      {!isAdminRoute && <Footer />}{!isAdminRoute && <HighgradeChatBot />}
    </div>
  </>;
};
